...
// Unified centering from both buildings and roads
...
// Update the BuildingMeshes component to render faces separately when in face edit mode
...
// This component uses R3F hooks and must be INSIDE the Canvas
function CanvasContent({
    ...
    const finishPolygon = () => {
        ...
        // Convert points to building vertices (apply centering offset)
        const vertices: [number, number][] = polygonPoints.map(([x, y]) => [
            x + centerX, 
            y + centerY
        ]);

        // Create new polygon building
        setBuildingList(prev => [
            ...prev,
            {
                vertices,
                height: 0.1, // Thin 2D surface
                isMain: false,
            }
        ]);
    ...
    useEffect(() => {
        ...
        const handlePointerUp = (event: PointerEvent) => {
            ...
            if (isDrawing && startPoint && currentPoint && activeTool === 'rectangle') {
                console.log('Completing rectangle'); // Debug log
                ...
                if (width > 0.5 && height > 0.5) {
                    // Get the center calculation that will be applied
                    const { centerX, centerY } = computeSceneCenter(
                        buildingList, 
                        roads
                    );
                    ...
                    setBuildingList(prev => [
                        ...
                        {
                            vertices,
                            height: 0.1, // Low height for 2D shape
                            isMain: false,
                        }
                    ]);
            ...
            if (isDrawing && startPoint && currentPoint && activeTool === 'circle') {
                console.log('Completing circle'); // Debug log
                // Calculate radius
                ...
                console.log('Circle radius:', radius); // Debug log
                ...
                if (radius > 0.5) {
                    ...
                    console.log('Circle vertices created:', vertices.length); // Debug log
...
export default function BuildingScene({
    ...
    // Add new states for floor creation
    ...
    // Helper to push current state to history before mutating
    const pushHistory = useCallback(() => {
        setHistory(prev => [
            ...prev,
            buildingList.map(b => ({
                ...b,
                vertices: b.vertices.map(v => [...v] as [number, number]),
                buffer: b.buffer ? b.buffer.map(v => [...v] as [number, number]) : undefined
            }))
        ]);
    }, [buildingList]);
    ...
    const handleCreateFloors = () => {
        ...
        // Create new floor buildings on top of the selected building
        const newBuildings: PolygonData[] = [];
        
        for (let i = 0; i < floors; i++) {
            const floorBuilding: PolygonData = {
                vertices: selectedBuilding.vertices.map(v => [...v] as [number, number]),
                height: height,
                isMain: false,
                buffer: selectedBuilding.buffer?.map(v => [...v] as [number, number]),
                // Add floor metadata
                floorLevel: i + 1,
        ...
        // Add the new floor buildings to the list
        setBuildingList(prev => [...prev, ...newBuildings]);
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === "Delete" && selected !== null) {
                pushHistory();
                setBuildingList(prev => prev.filter((_, i) => i !== selected));
                setSelected(null);
            }
        };
        window.addEventListener("keydown", handleKeyDown);
    ...
    // Handler for double-clicking a face
    const handleFaceDoubleClick = (buildingIdx: number, faceIndex: number, event: any) => {
        setEditMode({ buildingIdx, faceIndex });
    };
 
    // Push/Pull (extrude) the selected face parallel to itself
    const handlePushPull = (distance: number) => {
        if (!editMode) return;
        pushHistory();
        setBuildingList(prev =>
            prev.map((b, i) => {
                if (i !== editMode.buildingIdx) return b;
                const verts = b.vertices ?? [];
                if (verts.length < 2) return b;
                const vi1 = editMode.faceIndex;
                const vi2 = (vi1 + 1) % verts.length;
                const [x1, y1] = verts[vi1];
                const [x2, y2] = verts[vi2];
                ...
                const newVerts = verts.map((v, idx) => {
                    if (idx === vi1 || idx === vi2) {
                        return [v[0] + nx * distance, v[1] + ny * distance] as [number, number];
                    }
                    return v;
                });
        ...
        <>
            ...
            >
                ...
                <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    {/* Add this button to your toolbar - insert it right after the Polygon button */}
                    <button
                        style={{
                            padding: "8px 12px",
                    ...
                    {/* Add this button to your toolbar - add it after the Push/Pull button */}
                    <button
                        style={{
                            padding: "8px 12px",
